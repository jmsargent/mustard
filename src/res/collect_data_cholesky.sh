...
if [ $gpu_count -eq -1 ]
then 
    if [ $method -eq -1 ] || [ $method -eq 1 ]; then $MPI_HOME/bin/mpirun -n 1 $MCUDAGRAPH_DIR/chol_partg -N=$N -T=$T $verb -run=$runs >> chol_logs/$N/log0_$N.log ; fi
    if [ $method -eq -1 ] || [ $method -eq 2 ]; then $MPI_HOME/bin/mpirun -n 1 $MCUDAGRAPH_DIR/chol_partg -N=$N -T=$T -tiled $verb -workspace=256 -sm=20 -run=$runs >> chol_logs/$N/log1_$N\_$T.log ; fi
