...
if [ $gpu_count -eq -1 ]
then 
    if [ $method -eq -1 ] || [ $method -eq 1 ]; then $MPI_HOME/bin/mpirun -n 1 $MCUDAGRAPH_DIR/lu_partg -N=$N -T=$T $verb -run=$runs >> logs/$N/log0_$N.log ; fi
    # if [ $method -eq -1 ] || [ $method -eq 2 ]; then $MPI_HOME/bin/mpirun -n 1 $MCUDAGRAPH_DIR/lu_partg -N=$N -T=$T -tiled $verb -workspace=256 -sm=20 -run=$runs >> logs/$N/log1_$N\_$T.log ; fi
        
    # export CUDA_VISIBLE_DEVICES="0"
    # for ((g = 1 ; g <= 4 ; g++ )); do
    #     echo "$CUDA_VISIBLE_DEVICES"
    #     if [ $method -eq -1 ] || [ $method -eq 3 ]; then $MPI_HOME/bin/mpirun -n $g $MCUDAGRAPH_DIR/lu_partg -N=$N -T=$T -subgraph $verb -workspace=256 -sm=20 -run=$runs >> logs/$N/log2_$N\_$T\_$g\GPU.log  ; fi

    #     for ((i = 0 ; i < $runs ; i++ )); do
    #         if [ $method -eq -1 ] || [ $method -eq 4 ]; then $MG_SAMPLES_DIR/cusolver_MgGetrf_example >> logs/$N/log3_$N\_$T\_$g\GPU.log ; fi
    ...
    echo "$CUDA_VISIBLE_DEVICES"
    for ((i = 0 ; i < $runs ; i++ )); do
        if [ $method -eq -1 ] || [ $method -eq 4 ]; then $MG_SAMPLES_DIR/cusolver_MgGetrf_example ; fi    
