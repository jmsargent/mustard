...
void tiledLU(bool verify, bool subgraph, bool dot)
{
    ...
    int cublasWorkspaceSize = 256*256;
    ...
    cudaStream_t* cublasStreams = new cudaStream_t[workspaces];
    cublasHandle_t* cublasHandles = new cublasHandle_t[workspaces];
    for (int i = 0; i < workspaces; i++) {
        checkCudaErrors(cudaStreamCreate(&cublasStreams[i]));
        checkCudaErrors(cublasCreate(&cublasHandles[i]));
        checkCudaErrors(cublasSetStream(cublasHandles[i], cublasStreams[i]));
        checkCudaErrors(cublasSetWorkspace(cublasHandles[i], d_workspace_cublas[i], cublasWorkspaceSize));
    }
    ...
    for (int k = 0; k < T; k++)
    {
        // A[k][k] = GETRF(A[k][k])
        // L[k][k]*U[k][k] = A[k][k]
        tiledLUGraphCreator->setStream(s);
        ...
        for (int i = k + 1; i < T; i++)
        {
            ...
            // checkCudaErrors(cublasSetStream(cublasHandle, cublasStreams[i]));
            tiledLUGraphCreator->setStream(cublasStreams[i]);
        ...
        // checkCudaErrors(cublasSetStream(cublasHandle, cublasStreams[0]));
        tiledLUGraphCreator->setStream(cublasStreams[0]);
        //checkCudaErrors(cublasSetWorkspace(cublasHandle, d_workspace_cublas[0], cublasWorkspaceSize));
    ...
    if (subgraph) {
        ...
        for (int i = 0; i < totalNodes; i++)
        {
            ...
            //checkCudaErrors(cudaGraphUpload(h_subgraphsExec[i], s));
        }
