...
/* A [ m ] [ n ] */
double *A[NMAXBLOCKS][NMAXBLOCKS];
...
static int cholesky_no_stride(void)
{
	...
	double flop = (2.0f*size_p*size_p*size_p)/3.0f;
...
int main(int argc, char **argv)
{
	...
	for (n = 0; n < nblocks_p; n++)
	{
		if (n <= m)
		{
			starpu_malloc_flags((void **)&A[m][n], BLOCKSIZE*BLOCKSIZE*sizeof(double), STARPU_MALLOC_PINNED|STARPU_MALLOC_SIMULATION_FOLDED);
			assert(A[m][n]);
		}
	}
    ...
    size_t size = nblocks_p*BLOCKSIZE;
	PRINTF("SIZE = %d\n", (int)size);

    double *h_A = (double *)malloc(size * size * sizeof(double));

    generateRandomSymmetricPositiveDefiniteMatrix(h_A, size);
	...
	if (n <= m)
	{
		...
		{
			A[m][n][mm*BLOCKSIZE + nn] = h_A[index++];  
	...
	for (n = 0; n < nblocks_p; n++)
	{
		if (n <= m)
		{
			starpu_matrix_data_register(&A_state[m][n], STARPU_MAIN_RAM, (uintptr_t)A[m][n],
						    BLOCKSIZE, BLOCKSIZE, BLOCKSIZE, sizeof(double));
	...
	for (n = 0; n < nblocks_p; n++)
	{
		if (n <= m)
		{
			...
			starpu_free_flags(A[m][n], BLOCKSIZE*BLOCKSIZE*sizeof(double), STARPU_MALLOC_PINNED|STARPU_MALLOC_SIMULATION_FOLDED);
		}
	}
