...
static inline void chol_common_cpu_codelet_update_u22(void *descr[], int s, void *_args)
{
	(void)_args;
	/* printf("22\n"); */
	double *left 	= (double *)STARPU_MATRIX_GET_PTR(descr[0]);
	double *right 	= (double *)STARPU_MATRIX_GET_PTR(descr[1]);
	double *center 	= (double *)STARPU_MATRIX_GET_PTR(descr[2]);
	...
	if (s == 0)
	{
		...
		if (worker_size == 1)
		{
			...
			STARPU_DGEMM("N", "T", dy, dx, dz, -1.0f, left, ld21,
		...
		else
		{
			...
			double *new_left = &left[block_size*rank];
			double *new_center = &center[block_size*rank];

			STARPU_DGEMM("N", "T", dy, new_dx, dz, -1.0f, new_left, ld21,
...
/*	printf("21\n"); */
	...
	sub11 = (double *)STARPU_MATRIX_GET_PTR(descr[0]);
	sub21 = (double *)STARPU_MATRIX_GET_PTR(descr[1]);
	...
	{
		case 0:
			STARPU_DTRSM("R", "L", "T", "N", nx21, ny21, 1.0f, sub11, ld11, sub21, ld21);
			break;
#ifdef STARPU_USE_CUDA
		case 1:
			status = cublasDtrsm(starpu_cublas_get_local_handle(),
	...
	switch (s)
	{
		case 0:
			...
			for (z = 0; z < nx; z++)
			{
				...
				STARPU_DSCAL(nx - z - 1, 1.0f/lambda11, &sub11[(z+1)+z*ld], 1);

				STARPU_DSYR("L", nx - z - 1, -1.0f,
							&sub11[(z+1)+z*ld], 1,
							&sub11[(z+1)+(z+1)*ld], ld);
			}
#endif
		...
		case 1:
			...
			cudaHostAlloc((void **)&lambda11, sizeof(double), 0);

			for (z = 0; z < nx; z++)
			{
				cudaMemcpyAsync(lambda11, &sub11[z+z*ld], sizeof(double), cudaMemcpyDeviceToHost, stream);
...
/*				cublasSetVector(1, sizeof(float), lambda11, sizeof(float), &sub11[z+z*ld], sizeof(float)); */
				cudaMemcpyAsync(&sub11[z+z*ld], lambda11, sizeof(double), cudaMemcpyHostToDevice, stream);
				double scal = 1.0f/(*lambda11);
